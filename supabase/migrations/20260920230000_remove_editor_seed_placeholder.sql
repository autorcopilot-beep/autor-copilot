...
set content_html = ''
where content_html = '<p>Comece a escrever sua história aqui.</p>';
...
begin
  if caller_id is null or not exists (
    ...
    where id = target_work_id and owner_id = caller_id
  ) then
    raise exception 'Writing work is unavailable' using errcode = '42501';
  end if;

  perform pg_advisory_xact_lock(hashtextextended(target_work_id::text, 0));

  if not exists (
    select 1 from public.writing_documents
    where work_id = target_work_id and owner_id = caller_id
  ) then
    insert into public.writing_documents (
      work_id, owner_id, parent_id, kind, title, content_html,
      synopsis, status, word_goal, position
    ) values
      (target_work_id, caller_id, null, 'chapter', 'Capítulo 1', '', '', 'draft', 1800, 0),
