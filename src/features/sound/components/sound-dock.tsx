...
export function SoundDock({ enabled }: { enabled: boolean }) {
  ...
  return <aside className="fixed bottom-4 left-1/2 z-40 w-[min(42rem,calc(100vw-1.5rem))] -translate-x-1/2 overflow-hidden rounded-2xl border border-line-strong bg-surface/95 shadow-floating backdrop-blur-xl" aria-label="Reprodutor Media & Sound">
    ...
    <div className="flex min-h-16 items-center gap-3 px-3 sm:px-4">
      <Link href="/sound" className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-accent-subtle text-accent" title="Abrir Media & Sound"><Headphones className="size-5" /></Link>
      <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-ink">{sound.currentTrack?.title ?? 'Paisagem sonora'}</p><p className="truncate text-[11px] text-muted">{sound.currentTrack ? `${time(sound.currentTime)} · ${time(sound.duration)}` : 'Mixer Web Audio ativo'}</p></div>
      ...
      <Link href="/sound#mixer" aria-label="Abrir mixer" className="flex size-9 items-center justify-center rounded-full text-muted hover:bg-surface-muted hover:text-ink"><SlidersHorizontal className="size-4" /></Link>
